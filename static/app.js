...
$("analyzeBtn").onclick = async () => {
  ...
  try {
    ...
    $("signalSide").textContent = s.side;
    $("signalConfidence").textContent = `${s.confidence}/100`;
    $("signalRisk").textContent = s.risk_level || "—";
    $("longScore").textContent = s.long_score;
    $("shortScore").textContent = s.short_score;
    $("longBar").style.width = `${s.long_score}%`;
    $("shortBar").style.width = `${s.short_score}%`;
    $("entryValue").textContent = fmt(s.entry, 2);
    $("stopValue").textContent = fmt(s.stop, 2);
    $("targetValue").textContent = fmt(s.target, 2);
    $("rrValue").textContent = s.risk_reward ? `1:${fmt(s.risk_reward, 1)}` : "—";
    $("recommendation").textContent = s.recommendation || "";

    $("factorList").innerHTML = (s.factor_details || [])
      .map(x => `<div>${x}</div>`).join("");

    $("positionEntry").value = s.entry ?? "";
    $("positionStop").value = s.stop ?? "";
    $("signalCard").classList.remove("hidden");

    $("analysis").textContent =
`交易对：${j.contract}｜周期：${j.interval}
RSI14：${fmt(s.rsi, 2)}
ATR：${fmt(s.atr, 2)}（${fmt(s.atr_pct, 2)}%）
EMA20 / 50 / 200：${fmt(s.ema20,2)} / ${fmt(s.ema50,2)} / ${fmt(s.ema200,2)}
近期支撑 / 阻力：${fmt(s.support,2)} / ${fmt(s.resistance,2)}
核心原因：${(s.reasons || []).join("；") || "无强方向"}
...
$("dashboardBtn").onclick = async () => {
  $("dashboardResult").textContent = "正在读取…";
  try {
    const r = await fetch("/api/dashboard");
    const j = await r.json();
    if (!r.ok) throw new Error(j.detail || "读取失败");
    const x = j.summary;
    $("dashboardResult").textContent =
`已记录信号：${x.signals_logged}
LONG：${x.long_signals}
SHORT：${x.short_signals}
FLAT：${x.flat_signals}
高信心（≥80）：${x.high_confidence_signals}
平均信心：${x.average_confidence}

${j.notice}`;
  } catch (e) {
    $("dashboardResult").textContent = `错误：${e.message}`;
  }
};


$("positionBtn").onclick = async () => {
  $("positionResult").textContent = "正在计算…";
  const body = {
    account_balance: Number($("accountBalance").value),
    risk_fraction: Number($("positionRisk").value),
    entry: Number($("positionEntry").value),
    stop: Number($("positionStop").value),
    leverage: Number($("positionLeverage").value)
  };
  try {
    const r = await fetch("/api/position-size", {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify(body)
    });
...
`最大计划亏损：${fmt(x.max_loss, 2)} USDT
每单位价格风险：${fmt(x.risk_per_unit, 4)}
参考数量：${fmt(x.quantity, 6)}
参考名义价值：${fmt(x.notional, 2)} USDT
杠杆：${fmt(x.leverage, 1)}x
估算保证金：${fmt(x.estimated_margin, 2)} USDT

${j.notice}`;
  } catch (e) {
    $("positionResult").textContent = `错误：${e.message}`;
  }
};
